...
function toggleMenu() {
  // Delay para animação do botão
  setTimeout(() => {
    isActive.value = !isActive.value
  }, 500)

  // Mantém o resto do menu
  isOpen.value = !isOpen.value

  setTimeout(() => {
    menuTitle.value = !(container.value || (!container.value && isOpen.value))
  }, 200)
...
const submitContato = () => {
  ...
  const whatsappUrl = `https://wa.me/+5521936184024?text=${encodeURIComponent(message)}`
  window.location.href = whatsappUrl
}

const submitPersonal = () => {
  const message = `Olá! Gostaria de informações sobre Personal, me chamo   `
  const whatsappUrl = `https://wa.me/+5521936184024?text=${encodeURIComponent(message)}`
  window.location.href = whatsappUrl
}

function scrollToProjectsHome() {
  const isMobile = window.innerWidth <= 450
  const offset = isMobile ? 100 : 98

  // decide qual anchor usar
  const targetId = isMobile ? '#projects-mobile' : '#projects-desktop'

  // navega pra home com o id certo
  navigateTo(`/${targetId}`).then(() => {
    // espera a página montar e aí faz o smooth
    setTimeout(() => {
    ...
    }, 300) // tempo pra garantir que o DOM carregou
  })
}
...
.slide-menu-leave-active {
  transition: transform .5s cubic-bezier(.3,0,.5,1);;
}
...
.logo-circle.active {
  filter: brightness(0) invert(1); /* exemplo: transforma em branco */
}
...
.slide-menu-enter-from { transform: translateY(100%); }
.slide-menu-leave-to { transform: translateY(-100%); }
.slide-menu-enter-active,
.slide-menu-leave-active { transition: transform .5s cubic-bezier(.3,0,.5,1); }
.slide-menu-enter-to,
.slide-menu-leave-from { transform: translateY(0); opacity: 1; }

.menu-open .links a.isActive { color: #fff; text-decoration: underline; }
.menu-open .links a:hover { color: #fff; cursor: pointer; }
.site-footer{
  position: fixed;
  bottom: 0;
  left: 0;
...
@media(max-width: 600px){
    ...
    .logo-text h1{font-size:1.4rem; margin: 20px 30px -10px 0}
    .logo-text span{font-size:0.55rem; margin-left: 0px;}
